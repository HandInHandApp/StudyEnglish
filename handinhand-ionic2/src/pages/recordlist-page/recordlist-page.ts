...
export class RecordListPage implements AfterViewInit, OnDestroy {
     ...
     * @returns {void}
     */
    private deleteNodes(keyDict: KeyDict, node: TreeNode): void {
     ...
     * @returns {void}
     */
    private checkIfDeletingInOtherFolders(node: TreeNode): void {
            ...
            if (nSelectedNodesHere) {
                alertAndDo(
                    ...
                    () => {
                        ...
                        this.deleteNodes(this.selectedNodes,node);
                    },
                    'Delete only here (' + nSelectedNodesHere + ')',
                    () => {
                        console.log('yes action');
                        this.deleteNodes(selectedNodesHere,node);
                    }
                );
            }
            else {
     ...
     * @returns {void}
     */
    public onClickDeleteButton(node: TreeNode): void {
        this.onClickCheckbox(node);
        if (this.selectedNodes[UNFILED_FOLDER_KEY]) {
            ...
            alertAndDo(
                ...
                () => {
                    ...
                    this.checkIfDeletingInOtherFolders(node);
                }
            );
        }
        else {
